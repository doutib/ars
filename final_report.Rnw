...
\author{Name:Thibault Doutre SID :\\
Name:Paul Cho SID:19642164\\
...
Zhenyuan Liu focused on developing the algorithm for initialization, adaptive rejection sampling, and updating as well as documentation. Thibault Doutre focused on testing, unit testing and searching for x1 and xk in the initialization part. Hao Lyu focused on the searching for x1 and xk, checking for log concavity and explored different algorithms for sampling efficiently and accurately. Paul Cho focused on vectorizing the functions, doing the R packaging,and implementing the help manual and documentation.\\
